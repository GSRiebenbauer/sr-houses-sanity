...
export function ImageBlock({ content }) {
  ...
  return (
    <div>
      ...
      {layout === 'layout2' && (
        <div
          className={`${paddingClass} ${content.backgroundWhite ? 'bg-white' : ''}  image-block flex flex-col lg:grid lg:grid-cols-8 gap-x-8 px-5 lg:px-10`}
        >
          {images.map((image, index) => (
            <div
              key={image._key}
      ...
      {layout === 'layout4' && (
        <div
          className={` ${paddingClass} ${content.backgroundWhite ? 'bg-white' : ''} image-block flex flex-col lg:grid lg:grid-cols-8 lg:h-screen `}
        >
          {images.map((image, index) => (
            <div
              key={image._key}
